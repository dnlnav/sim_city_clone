...
type MetaDataType = { height: number };
...
const assets: {
	...
	residential: (x, y, { height } = { height: 1 }) => {
	...
	commercial: (x, y, { height } = { height: 1 }) => {
	...
	industrial: (x, y, { height } = { height: 1 }) => {
...
export const createAssetInstance = (
	...
	x: number,
	y: number,
	data: MetaDataType = { height: 1 }
) => {
	return assets?.[id](x, y, data);
};
